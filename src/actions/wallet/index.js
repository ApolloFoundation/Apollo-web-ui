...
export function getWallets(requestParams) {
  return dispatch => handleFetch(`${config.api.server}/rest/keyStore/accountInfo`, POST, requestParams, { isJson:  true })
    .then(async res => {
      if (!res.errorCode) {
...
export function getContractStatus(requestParams) {
  return (dispatch, getState) => {
    ...
    return handleFetch(`${config.api.server}/rest/dex/contracts`, GET, requestParams, { isJson:  true })
      .then(res => {
        if (!res.errorCode) {
...
export function getAllContractStatus(requestParams) {
  return (dispatch, getState) => {
    ...
    return handleFetch(`${config.api.server}/rest/dex/all-contracts`, GET, requestParams, { isJson:  true })
      .then(res => {
        if (!res.errorCode) {
...
export function logout(requestParams) {
  return () => handleFetch(`${config.api.server}/rest/dex/flush`, GET, requestParams, { isJson:  true })
    .then(res => {
      if (res.errorCode) {
        NotificationManager.error(res.errorDescription, 'Error', 5000);
      }
...
export function getCurrencyBalance(requestParams) {
  return () => handleFetch(`${config.api.server}/rest/dex/balance`, GET, requestParams, { isJson:  true })
    .then(res => {
      if (!res.errorCode) {
        return res;
      }
...
export function walletWithdraw(requestParams) {
  return () => handleFetch(`${config.api.server}/rest/dex/withdraw`, POST, requestParams, { isJson:  true })
    .then(async res => {
      if (!res.errorCode) {
        return res;
      }
...
export function createOffer(requestParams) {
  ...
  return dispatch => handleFetch(`${config.api.server}/rest/dex/offer`, POST, params, { isJson:  true })
    .then(async res => {
      if (!res.errorCode) {
...
export function cancelOffer(requestParams) {
  return dispatch => handleFetch(`${config.api.server}/rest/dex/offer/cancel`, POST, requestParams, { isJson:  true })
    .then(async res => {
      if (!res.errorCode) {
...
export function getOrderById(orderId) {
  return () => handleFetch(`${config.api.server}/rest/dex/orders/${orderId}`, GET, null, { isJson:  true })
    .then(async res => {
      if (!res.errorCode) {
        return res;
      }
...
export function getOpenOrders(requestParams) {
  return () => handleFetch(`${config.api.server}/rest/dex/offers`, GET, requestParams, { isJson:  true })
    .then(async res => {
      if (!res.errorCode) {
        return res;
      }
...
export function getTransactionFee() {
  return () => handleFetch(`${config.api.server}/rest/dex/ethInfo`, GET, null, { isJson:  true })
    .then(async res => {
      if (!res.errorCode) {
        return res;
      }
...
export function exportWallet(requestParams) {
  return () => handleFetch(`${config.api.server}/rest/keyStore/eth`, POST, requestParams, { isJson:  true })
    .then(async res => {
      if (!res.errorCode) {
        return res;
      }
