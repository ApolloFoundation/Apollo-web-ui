...
function getPrivateKeyAPL(sp) {
  ...
  return conv.shortArrayToHexString(
    curve25519_clampAPL(conv.byteArrayToShortArray(bts))
  );
}
...
const validatePassphrase = (passphrase) => (dispatch, getStore) =>
...
function getSharedSecret(key1, key2) {
  return conv.shortArrayToByteArrayAPL(
    curve25519_(
      conv.byteArrayToShortArray(key1),
      conv.byteArrayToShortArray(key2),
      null
    )
  );
}
...
function decryptMessage(data, options) {
  ...
  return {
    message: message,
    sharedKey: conv.byteArrayToHexString(result.sharedKey),
  };
}
...
function decryptNoteAPL(m, o, sp) {
  try {
    if (!o.sharedKey) {
      ...
      if (!o.publicKey) {
        o.publicKey = conv.hexStringToByteArray(
          getPublicKeyAPL(o.account, true)
        );
      }
    }
    if (o.nonce) {
