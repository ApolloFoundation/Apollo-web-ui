...
class InfoLedgerTransaction extends React.Component {
  ...
  processTransaction = async () => {
    const transaction = await this.props.getTransaction({
    ...
    if (
      ...
      if (props.modalData && !props.modalData.errorCode) {
        ...
        return {
          ...
          transactionId: isDataObject
            ? props.modalData.transaction
            : props.modalData,
  ...
  componentDidMount() {
    if (
      ...
      !(this.state.transaction instanceof Object)
    ) {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.state.transactionId &&
      prevState.transactionId !== this.state.transactionId
    ) {
      this.processTransaction();
    }
    if (
      ...
      this.state.transaction !== prevState.transaction
    ) {
  ...
  getWhiteListOfTransaction = () => {
    if (
      this.state.transaction &&
      this.state.transaction.attachment.phasingWhitelist
    ) {
      const whitelist = this.state.transaction.attachment.phasingWhitelist.map(
        (el) => {
      ...
      Promise.all(whitelist).then((data) => {
        this.setState({
          whitelist: data,
        });
      });
  ...
  render() {
    ...
    const parsedSignatures =
      ...
      (typeof transaction.signature === "string"
        ? [transaction.signature]
        : transaction.signature.signatures.map((i) => i.signature));
    const recipientRS =
      transaction &&
      (this.props.accountRS === transaction.recipientRS
    ...
    return (
      ...
      >
        {!this.state.isPending ? (
          transaction &&
          this.props.constants.transactionTypes &&
          this.props.constants.transactionTypes[transaction.type] ? (
            <TabulationBody>
              <TabContaier sectionName={"Info"}>
                <div className="transaction-table no-min-height transparent">
                  <InfoTransactionTable
                  ...
                  <div className="flexible-grid">
                    <button
                      ...
                      className={`btn btn-green ${
                        !this.state.transaction.recipientRS ? "disabled" : ""
                      }`}
                    >
                      Send Apollo
                    </button>
                    <button
                      type={"button"}
                      onClick={() =>
                        this.props.setBodyModalParamsAction("COMPOSE_MESSAGE", {
                          recipient: recipientRS,
                        })
                      }
                      className={`btn btn-green ${
                ...
                <div className="transaction-table no-min-height transparent">
                  <div className="transaction-table-body transparent full-info">
                    <table>
                      <tbody>
                        {this.state.transaction &&
                          this.state.transaction.errorMessage && (
                            <tr>
                              <td>Error message:</td>
                              <td>
                                {this.state.transaction.errorMessage ||
                        ...
                        <tr>
                          <td>Full Hash:</td>
                          <td className={"no-white-space break-word"}>
                        ...
                        <tr>
                          <td>Phased:</td>
                          <td>
                            {this.state.transaction.phased ? "true" : "false"}
                          </td>
                        </tr>
                        <tr>
                          <td>EC block id:</td>
                          <td className={"no-white-space break-word"}>
                            {this.state.transaction.ecBlockId}
                          </td>
                        </tr>
                        <tr>
                          ...
                          <td className={"no-white-space"}>
                            <span className={"break-word"}>
                              {this.state.transaction.signatureHash}
                            </span>
                          </td>
                        </tr>
                        ...
                        <table>
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td>
                                {this.state.transaction.attachment
                                  .phasingVotingModel === 0 && "ACCOUNT"}
                                {this.state.transaction.attachment
                                  .phasingVotingModel === 1 &&
                                  "ACCOUNT BALANCE"}
                                {this.state.transaction.attachment
                                  .phasingVotingModel === 2 && "ASSET BALANCE"}
                                {this.state.transaction.attachment
                            ...
                            <tr>
                              ...
                              <td>
                                ...
                                >
                                  <div
                                    className={
                                      "transaction-table-body transparent no-border-top"
                                    }
                                  >
                                    <table>
                                      <thead>
            ...
            <InfoBox default>Transaction not found</InfoBox>
          )
        ) : (
          <ContentLoader />
        )}
