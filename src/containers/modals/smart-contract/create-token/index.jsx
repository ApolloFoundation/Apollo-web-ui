...
const CreateToken = ({ closeModal }) => {
  ...
  const type = useSelector((state) => state.modals.modalData);
  ...
  const getStateTokenList = useCallback(
    async (type) => {
      const { modules } = await dispatch(getTokenList(type));
      if (modules) {
