...
class UserBox extends Component {
  ...
  handleClickOutside = (event) => {
    if (
      ...
      this.refSearchInput &&
      !this.refSearchInput.current.contains(event.target)
    ) {
      this.setState({
        searching: false,
      });
  ...
  handleSearchind = async (values) => {
    if (!this.state.isSearching) {
      ...
      const transaction = this.props.getTransactionAction({
        transaction: values.value,
      });
      const block = this.props.getBlockAction({ block: values.value });
      const account = this.props.getAccountInfoAction({
        account: values.value,
      });

      Promise.all([transaction, block, account]).then((data) => {
              ...
              <form onSubmit={submitForm}>
                <div className="user-account-actions">
                  <CopyToClipboard
                    ...
                    onCopy={() => {
                      NotificationManager.success(
                        "The account has been copied to clipboard."
                      );
                    }}
                  >
                    <Button
                  ...
                  <div
                    ...
                    className={"searching-window-wrap"}
                  >
                    <div className={"searching-window-slide"}>
                      <div className={"searching-window-icon"}>
                        <IconndeButton
                          ...
                          action={() =>
                            this.setSearchStateToActive(getFormState().values)
                          }
                        />
                      </div>
                      <Text
