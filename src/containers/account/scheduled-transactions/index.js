...
class ScheduledTransactions extends React.Component {
  state = {
    ...
    adminPassword: localStorage.getItem("adminPassword")
      ? JSON.parse(localStorage.getItem("adminPassword"))
      : "",
  };
  ...
  componentDidUpdate = () => {
    if (
      !this.state.isLoaded &&
      this.props.account &&
      (this.state.adminPassword || this.state.adminPassword === "")
    ) {
  ...
  deleteScheduledTransaction = async (transaction) => {
    const deleteTransacrtio = await this.props.submitForm(
      { adminPassword: this.state.adminPassword, transaction },
    ...
    if (deleteTransacrtio) {
      if (deleteTransacrtio.errorCode) {
        NotificationManager.error(
          deleteTransacrtio.errorDescription,
          "Error",
          5000
        );
      } else {
        NotificationManager.success(
          "Scheduled transaction has been deleted!",
          null,
          5000
        );
          ...
          <div className="scheduled-transactions">
            {this.state.scheduledTransactions &&
              ...
              this.state.scheduledTransactions.errorCode === 3 && (
                <InfoBox default>
                  An admin password was not specified. Please set an admin
                  password on the
