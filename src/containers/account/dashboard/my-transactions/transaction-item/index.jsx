...
export default function TransactionItem(props) {
  ...
  const { constants, actualBlock, account, accountRS, decimals } = useSelector(
    (state) => state.account
  );
  ...
  const newRecipientRS =
    recipientRS && recipientRS === accountRS ? "You" : recipientRS;
  const transactionType =
    hasSubtype &&
    formatTransactionType(
      constants.transactionTypes[type].subtypes[subtype].name
    );
  ...
  const isSmcTransaction = !!/^SMC/.test(transactionType);
  const transactionTypeContent = useCallback(
    (condition) => {
      if (condition) {
        return newSenderRS;
      }
  ...
  const transactionInfo = useMemo(() => {
    ...
    if (isSmcTransaction) {
      return (
        <div className="transaction-type text-ellipsis">SMART CONTRACT</div>
      );
    }
    if (recipientRS) {
      return <span>{newRecipientRS}</span>;
    }
  ...
  const handleFailModalOpen = useCallback(
    (e) => {
  ...
  return (
    <div
      ...
      onClick={async () =>
        dispatch(
          setBodyModalParamsAction("INFO_TRANSACTION", props.transaction)
        )
      }
    >
      <div className="transaction-box">
          ...
          {!errorMessage && (
            <div
              className={classNamse({
                ...
                success:
                  account !== sender ||
                  (isDexOrder && attachment.offerCurrency !== 0),
                danger:
                  account === sender ||
                  (isDexOrder && attachment.offerCurrency === 0),
              })}
            >
              {isDexOrder
                ? `${attachment.offerCurrency === 0 ? "-" : ""}${
                    attachment.offerAmount / decimals
                  }`
                : `${account === sender ? "-" : ""}${
                    (amountATM === "0" &&
                    attachment.priceATM &&
        ...
        <div className="transaction-rs-wrap d-flex justify-content-between align-items-center">
          <div
            className={
              newSenderRS === "You"
                ? "transaction-you mr-2"
                : "transaction-rs text-ellipsis mr-2"
            }
          >
            {isDexOrder
          ...
          <div
            className={
              newRecipientRS === "You"
                ? "transaction-you ml-2"
                : "transaction-rs text-ellipsis ml-2"
            }
          >
            {transactionInfo}
          </div>
