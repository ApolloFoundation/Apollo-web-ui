...
const stopMonitorAction = (props) => {
  ...
  if (monitor && !monitor.errorCode) {
    NotificationManager.success(
      "The funding monitor has been stopped.",
      null,
      5000
    );
    setTimeout(() => {
  ...
  <tr>
    <td>
      <Button
        color="blue-link"
        onClick={() =>
          props.setBodyModalParamsAction("INFO_ACCOUNT", props.accountRS)
        }
        name={props.accountRS}
      />
    </td>
    <td className={"align-left"}>{props.property ? props.property : "?"}</td>
    <td className={"align-right"}>
      {props.amount ? props.amount / props.decimals : "?"}
    </td>
    <td>{props.threshold ? props.threshold / props.decimals : "?"}</td>
    <td>{props.interval ? props.interval : "?"}</td>
