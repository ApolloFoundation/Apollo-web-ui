...
class MyAssetItem extends React.Component {
  ...
  render() {
    const { decimals, asset, name, unconfirmedQuantityATU, quantityATU } =
      this.props;
    ...
    return (
      <tr key={uuidv4()}>
        ...
        <td className="align-right">
          {(unconfirmedQuantityATU / Math.pow(10, decimals)).toLocaleString(
            "en",
            {
        ...
        <td className="align-right">
          {(
            (parseInt(unconfirmedQuantityATU) / parseInt(quantityATU)) *
            100
          ).toFixed(2)}
          &nbsp;%
        </td>
        <td className="align-right">
          {!!(
            (this.state.lowestAskOrder / Math.pow(10, 8)) *
        ...
        <td className="align-right">
          {!!(
            (this.state.highestBidOrder / Math.pow(10, 8)) *
            Math.pow(10, decimals)
          ) &&
            (
              (this.state.highestBidOrder / Math.pow(10, 8)) *
          ...
          {!this.props.info && (
            <div className="btn-box inline">
              <button
                type={"button"}
                onClick={() =>
                  this.props.setBodyModalParamsAction("TRANSFER_ASSET", {
              ...
              <button
                type={"button"}
                onClick={() =>
                  this.props.setBodyModalParamsAction("DELETE_SHARES", {
                    quantityATU: quantityATU / Math.pow(10, decimals),
                    assetID: asset,
                    decimals,
                    assetName: name,
